...
sudo docker run --rm --gpus all --shm-size=16g -e WANDB_API_KEY=$WANDB_API_KEY proma2 grpo ++trainer.experiment_name=grpo-lr7 ++actor_rollout_ref.actor.optim.lr=7e-6 ++trainer.total_training_steps=102 
sudo fuser -k /dev/nvidia* 2>/dev/null || true
